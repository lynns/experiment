...
/**
 * Return the active variant for this feature given the context,
 * if none are active then it returns false.
 */
function feature(name, context) {
    var experiment = Experiment.byName(name);

    // Return if there is no experiment with the given name.
    if (!experiment) {
        exports.log('WARNING: Undefined experiment "' + name + '" was used.');
        return false;
    }

    var variant = context.variant(experiment);

    return !variant ? false : variant.name;
}
