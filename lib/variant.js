...
function Variant(props) {
    props = props || {};
    this.id = props.id;
    this.experimentId = props.experiment_id || props.experimentId;
...
Variant.findByExperimentName = function findByExperimentName(experimentName, callback) {
    var sql = "SELECT * FROM variants v, experiments e WHERE e.id=v.experiment_id AND e.name=?";

    db.query(sql, [experimentName], function (err, result) {
        if (err) {
            callback(err);
        } else if (result.length > 0) {
            var variants = result.map(function (props) {
...
Variant.prototype.__defineGetter__("prettyConversionRate", function () {
    return sprintf("%4.2f%%", this.conversionRate * 100);
});
